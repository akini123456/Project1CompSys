...
\texttt{part1}, the child loop still uses the assignment's 10-second sleep per
iteration, but it uses a bounded logical PID instead of the real Linux PID. The
logical PID is \texttt{child\_index + 1}, so the loop computes the sum from 0 to
\texttt{10 * logical\_pid}. This keeps the program runnable because real system PIDs
can be very large.
...
In fast mode, the program computes the same sum without the long per-iteration
delay.
...
The run targets generated output files in \path{outputs/}. I checked that the output
includes the expected \texttt{waitpid()} status lines, signal handler lines, pending
queue lines, and rule labels. The direct children are waited for, so no intended
child processes should remain as zombies after completion.
